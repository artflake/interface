...
describe('prices', () => {
  ...
  const pair12 = new Pair(CurrencyAmount.fromRawAmount(token1, JSBI.BigInt(10000)), CurrencyAmount.fromRawAmount(token2, JSBI.BigInt(20000)));
  const pair23 = new Pair(CurrencyAmount.fromRawAmount(token2, JSBI.BigInt(20000)), CurrencyAmount.fromRawAmount(token3, JSBI.BigInt(30000)));
  describe('#computeRealizedLPFeeAmount', () => {
    ...
    it('correct realized lp fee for single hop', () => {
      expect(computeRealizedLPFeeAmount(new Trade(new Route([pair12], token1, token2), CurrencyAmount.fromRawAmount(token1, JSBI.BigInt(1000)), TradeType.EXACT_INPUT))).toEqual(CurrencyAmount.fromRawAmount(token1, JSBI.BigInt(3)));
    });
    it('correct realized lp fee for double hop', () => {
      expect(computeRealizedLPFeeAmount(new Trade(new Route([pair12, pair23], token1, token3), CurrencyAmount.fromRawAmount(token1, JSBI.BigInt(1000)), TradeType.EXACT_INPUT))).toEqual(CurrencyAmount.fromRawAmount(token1, JSBI.BigInt(5)));
    });
  });
